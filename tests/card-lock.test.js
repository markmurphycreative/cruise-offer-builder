...
test('offer selector tiles include native lock toggles and locked styling', () => {
  ...
  assert.match(html, /\.offer-lock-toggle\{[^}]*top:auto[^}]*right:auto[^}]*left:50%[^}]*bottom:6px[^}]*width:10px[^}]*height:10px[^}]*padding:0[^}]*background:transparent/);
  ...
  assert.match(html, /\.offer-lock-toggle\.locked svg\{[^}]*width:8\.4px[^}]*height:8\.4px[^}]*stroke-width:2\.35[^}]*flex:0 0 8\.4px/);
