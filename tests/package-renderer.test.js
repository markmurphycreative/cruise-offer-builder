...
test('Package operator logos use operator-specific natural-aspect placement classes', () => {
  ...
  assert.match(html, /\.pc\.pkg-jet2\.pkg-jet2-couples \.pkg-operator-logo--jet2\{[^}]*left:var\(--pkg-left\);[^}]*top:581\.5px;[^}]*width:506px;[^}]*transform:translateY\(-50%\);[^}]*\}/);
...
test('Jet2 operator logo renderer output and live package selector support visible details placement', () => {
  ...
  assert.equal(computed.left, 'var(--pkg-left)');
  assert.equal(computed.top, '581.5px');
