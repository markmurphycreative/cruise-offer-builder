...
function createContext(){ const context={console, document:{getElementById(){return null;}}, clampParseConfidenceScore:v=>Math.max(0,Math.min(100,Number(v)||0))}; vm.createContext(context); vm.runInContext([
  ...
  extractFunction('hasDawsonIdentity'), extractFunction('isTrustedJet2DawsonQuote'), extractFunction('collectPackageOperatorEvidence'), extractFunction('getPackageOperatorMatch'), extractFunction('detectPackageOffer'), extractFunction('normalisePackageOcrText'), extractFunction('titleCasePackageValue'), extractFunction('detectPackageBoardBasis'), extractFunction('extractPackageDepartureAirport'), extractFunction('extractPackageSharingBasis'), extractFunction('normalisePackageMonetaryToken'), extractFunction('extractPackagePrices'), extractFunction('isUnsafePackageTitleLine'), extractFunction('cleanPackageParsedTitle'), extractFunction('sanitisePackageHotelCandidate'), extractFunction('extractHolidaySummaryAccommodation'), extractFunction('extractStructuredPackageAccommodation'), extractFunction('extractHolidayToAccommodation'), extractFunction('extractPriorityPackageHotel'), extractFunction('parseJet2DawsonQuote'), extractFunction('isJet2SourceInclusionCopy'), extractFunction('normaliseJet2PackageInclusionCopy'), extractFunction('parsePackageOfferText'), extractFunction('canApplyParsedPackageOffer')
...
test('Load Selected Imports route persists Jet2 package classification for operatorless package screenshots', () => {
  ...
  const context = {
    ...
    createPackageOfferId(){ return `test-package-${Math.random()}`; }, publishImportedOfferCollection(collection){ context.offers=collection; }, clearHeroImageDataFromOffer(){}, defaultTopBarUspForOperator(){ return ''; }, applyOperatorTopBarUspDefault(){},
  ...
  vm.runInContext([
    ...
    extractFunction('hasDawsonIdentity'), extractFunction('isTrustedJet2DawsonQuote'), extractFunction('collectPackageOperatorEvidence'), extractFunction('getPackageOperatorMatch'), extractFunction('detectPackageOffer'), extractFunction('detectOfferType'), extractFunction('normaliseVisionExtractedText'), extractFunction('normalisePackageOcrText'), extractFunction('titleCasePackageValue'), extractFunction('detectPackageBoardBasis'), extractFunction('extractPackageDepartureAirport'), extractFunction('extractPackageSharingBasis'), extractFunction('normalisePackageMonetaryToken'), extractFunction('extractPackagePrices'), extractFunction('isUnsafePackageTitleLine'), extractFunction('cleanPackageParsedTitle'), extractFunction('sanitisePackageHotelCandidate'), extractFunction('extractHolidaySummaryAccommodation'), extractFunction('extractStructuredPackageAccommodation'), extractFunction('extractHolidayToAccommodation'), extractFunction('extractPriorityPackageHotel'), extractFunction('parseJet2DawsonQuote'), extractFunction('isJet2SourceInclusionCopy'), extractFunction('normaliseJet2PackageInclusionCopy'), extractFunction('parsePackageOfferText'), extractFunction('canApplyParsedPackageOffer'), extractFunction('validatePackageScreenshotParse'), extractFunction('parseScreenshotTextForActiveBuilder'),
...
£515`;
  ...
  assert.equal(context.getScreenshotImportOperatorStatus(context.pendingScreenshotImports[1]), '');
...
7 nights from 9th July 2026`, {}).parsed;
  ...
  assert.equal(context.extractPriorityPackageHotel(`Your holiday to...
Servatur Castillo De Sol
Puerto Rico, Gran Canaria
Holiday summary
Accommodation
Servatur Castillo De Sol`.split(/\n+/).map(line => line.trim()).filter(Boolean), ['Puerto Rico, Gran Canaria']).hotel, 'Servatur Castillo De Sol');
...
Self Catering`, {}).parsed;
  ...
  assert.equal(context.extractPriorityPackageHotel(`Your holiday to Servatur Castillo De Sol Puerto Rico, Gran Canaria Departs: Thu 9th Jul 2026 at 06:00 Price payable £1,030`.split(/\n+/).map(line => line.trim()).filter(Boolean), []).hotel, 'Servatur Castillo De Sol');
