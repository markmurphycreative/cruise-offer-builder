...
test('the application version is defined once and hydrates every displayed version label', () => {
  assert.equal((html.match(/const APP_VERSION = "v2\.0\.0";/g) || []).length, 1);
  assert.equal((html.match(/data-app-version(?=[ >])/g) || []).length, 2);

  const { context, labels } = runVersionBootstrap();
  assert.equal(context.APP_VERSION, 'v2.0.0');
  assert.equal(context.document.title, 'Cruise Offer Builder v2.0.0 — Dawson & Sanderson');
  assert.deepEqual(labels.map(label => label.textContent), ['v2.0.0', 'v2.0.0']);
});

test('changing only APP_VERSION updates the title and every version label', () => {
  const changedHtml = html.replace('const APP_VERSION = "v2.0.0";', 'const APP_VERSION = "v9.9.9";');
