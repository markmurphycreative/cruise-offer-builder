...
test('the application version is defined once and hydrates every displayed version label', () => {
  assert.equal((html.match(/const APP_VERSION = "v\d+\.\d+\.\d+";/g) || []).length, 1);
  assert.equal((html.match(/data-app-version(?=[ >])/g) || []).length, 2);

  const { context, labels } = runVersionBootstrap();
  assert.equal(context.document.title, `Cruise Offer Builder ${context.APP_VERSION} — Dawson & Sanderson`);
  assert.deepEqual(labels.map(label => label.textContent), [context.APP_VERSION, context.APP_VERSION]);
});

test('changing only APP_VERSION updates the title and every version label', () => {
  const changedHtml = html.replace(/const APP_VERSION = "v\d+\.\d+\.\d+";/, 'const APP_VERSION = "v9.9.9";');
